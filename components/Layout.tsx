...
export const Layout: React.FC<LayoutProps> = ({ children, currentPage, onNavigate, headerExtra }) => {
  ...
  const languages: { code: Language; label: string; flagUrl: string }[] = [
    ...
    { code: 'ha', label: 'Hausa', flagUrl: 'https://flagcdn.com/ng.svg' },
    { code: 'yo', label: 'Yoruba', flagUrl: 'https://flagcdn.com/ng.svg' },
    { code: 'ig', label: 'Igbo', flagUrl: 'https://flagcdn.com/ng.svg' },
  ];
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 overflow-hidden text-slate-900 dark:text-slate-100 transition-colors" dir={dir}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 h-16 flex items-center justify-between px-4 lg:px-8 relative z-20 transition-colors">
          <button 
            ...
            className={`lg:hidden p-2 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 ${dir === 'rtl' ? '-mr-2' : '-ml-2'}`}
          >
            <Menu size={24} />
          </button>
          ...
          <div className="flex items-center flex-1 mx-2 lg:mx-6 max-w-2xl gap-4">
            ...
            {/* Desktop Search */}
            <div className="hidden md:block flex-1 relative" ref={searchContainerRef}>
            ...
            {/* Mobile Search Icon */}
            <button 
              className="md:hidden p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors ml-auto"
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            ...
            <div className="relative">
                <button 
                    ...
                    className="flex items-center gap-2 p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors"
                >
                    <img src={languages.find(l => l.code === language)?.flagUrl || 'https://flagcdn.com/us.svg'} alt={language} className="w-5 h-4 object-cover rounded-sm shadow-sm" />
                    ...
                    <div className={`absolute top-full mt-2 w-40 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-20 overflow-hidden ${dir === 'rtl' ? 'left-0' : 'right-0'}`}>
                        {languages.map((lang) => (
                            <button
                                ...
                                className={`w-full text-left px-4 py-3 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-3 ${language === lang.code ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-700 dark:text-slate-300'}`}
                            >
                                <img src={lang.flagUrl} alt={lang.label} className="w-5 h-4 object-cover rounded-sm shadow-sm" />
        ...
        {mobileSearchOpen && (
          <div className="md:hidden fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-start justify-center pt-4 px-4">
                  ...
                  searchResults.length > 0 ? (
                    searchResults.map((result) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-between group transition-colors rounded-xl"
                      >
                        <div className="flex items-center gap-3 overflow-hidden">
                          <div className={`p-2 rounded-lg flex-shrink-0 ${
                              result.type === 'PAGE' ? 'bg-slate-100 dark:bg-slate-700 text-slate-500' :
      ...
      {user?.role === 'FARMER' && currentPage !== 'support' && (
        <button
          onClick={() => onNavigate('support')}
          className="fixed bottom-6 right-6 bg-emerald-600 text-white p-4 rounded-full shadow-lg hover:bg-emerald-500 hover:shadow-xl transition-all transform hover:-translate-y-1 z-40 flex items-center justify-center group"
        >
          <Headset size={24} />
