...
export const CropDashboard: React.FC<CropDashboardProps> = ({ initialCropId, farmId }) => {
  ...
  const fetchCrops = async () => {
    try {
      const data = await ApiService.getCrops(farmId || undefined);
      setCrops(data);
    } catch (e) {
  ...
  const handleAddCrop = async () => {
    if (!farmId || !newCropData.name || !newCropData.type || !newCropData.fieldId || !newCropData.plantingDate) return;
    
    const newCrop: CropData = {
      id: `crop-${Date.now()}`,
      farmId,
      name: newCropData.name,
      type: newCropData.type,
      fieldId: newCropData.fieldId,
      ...
      alerts: [],
      history: [
        { date: new Date().toISOString(), ndvi: 0.8 }
      ]
    };

    try {
      await ApiService.createCrop(newCrop);
      await fetchCrops();
      setShowAddCropModal(false);
      setNewCropData({ name: '', type: '', fieldId: '', plantingDate: '' });
      addNotification({ title: 'Crop Added', message: 'New crop has been added successfully.', type: 'SUCCESS' });
    } catch (e) {
      console.error("Failed to add crop", e);
      addNotification({ title: 'Error', message: 'Failed to add crop field.', type: 'CRITICAL' });
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-lg font-semibold text-slate-800 dark:text-white">{t('my_fields')}</h2>
            <button 
              onClick={() => setShowAddCropModal(true)}
              className="text-sm bg-emerald-100 text-emerald-700 hover:bg-emerald-200 px-3 py-1 rounded-md font-medium transition-colors"
            >
              + Add Crop
            </button>
          </div>
          {displayedCrops.length === 0 ? (
            <div className="p-6 text-center bg-slate-50 dark:bg-slate-800 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
              <Sprout className="mx-auto h-8 w-8 text-slate-400 mb-2" />
              <p className="text-sm text-slate-500 dark:text-slate-400">No crops added yet.</p>
            </div>
          ) : (
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div 
                    ...
                    className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-6 flex flex-col items-center justify-center min-h-[200px] bg-slate-50 dark:bg-slate-700 relative cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                >
                  {analysisImage ? (
                    <img 
                      src={analysisImage} 
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {analysisResult.recommendations.map((rec, i) => (
                          <li key={i} className="flex items-start space-x-2 text-sm bg-slate-50 dark:bg-slate-700 p-2 rounded text-slate-700 dark:text-slate-300">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700">
              <h2 className="text-xl font-bold text-slate-800 dark:text-white">Add New Crop</h2>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Crop Name</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Crop Type</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Planting Date</label>
                <input 
                  type="date" 
                  value={newCropData.plantingDate}
                  onChange={e => setNewCropData({...newCropData, plantingDate: e.target.value})}
                  className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white"
                />
              </div>
            </div>
            <div className="p-6 border-t border-slate-100 dark:border-slate-700 flex justify-end space-x-3">
              ...
              <button 
                ...
                disabled={!newCropData.name || !newCropData.type || !newCropData.fieldId || !newCropData.plantingDate}
